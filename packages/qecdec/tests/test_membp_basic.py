"""Basic correctness tests for MemBPDecoder."""
...
def test_factory_registration(rsc_d5_data):
    assert "MemBP" in ALL_DECODERS
    assert "MemBP" in ITERATIVE_DECODERS
    dec = create_decoder(
        "MemBP",
        pcm=rsc_d5_data["pcm"],
    ...
    assert isinstance(dec, MemBPDecoder)
...
def test_all_zero_syndrome_fast_path(rsc_d5_data):
    dec = MemBPDecoder(rsc_d5_data["pcm"], rsc_d5_data["prior"], gamma=0.5, max_iter=20)
...
def test_gamma_zero_matches_standard_bp(rsc_d5_data):
    """MemBP with gamma = 0 reduces to standard BP."""
    mem = MemBPDecoder(
        rsc_d5_data["pcm"],
        rsc_d5_data["prior"],
        gamma=0.0,
    ...
    bp = BPDecoder(rsc_d5_data["pcm"], rsc_d5_data["prior"], max_iter=30, norm=0.9)
    syndromes = rsc_d5_data["syndromes"]
    ehat_mem, conv_mem, iters_mem = mem.decode_batch_detailed(syndromes)
    ehat_bp, conv_bp, iters_bp = bp.decode_batch_detailed(syndromes)
    np.testing.assert_array_equal(ehat_mem, ehat_bp)
    np.testing.assert_array_equal(conv_mem, conv_bp)
    np.testing.assert_array_equal(iters_mem, iters_bp)


def test_matches_dmembp_with_uniform_gamma(rsc_d5_data):
    """MemBP(gamma=g) must be bit-identical to DMemBP(gamma=g * ones)."""
    g = 0.5
    mem = MemBPDecoder(
        rsc_d5_data["pcm"],
        rsc_d5_data["prior"],
    ...
    dmem = DMemBPDecoder(
        rsc_d5_data["pcm"],
        rsc_d5_data["prior"],
        gamma=np.full(rsc_d5_data["num_vars"], g, dtype=np.float64),
        max_iter=30,
        norm=0.9,
    )
    syndromes = rsc_d5_data["syndromes"]
    ...
    np.testing.assert_array_equal(ehat_mem, ehat_dmem)
    np.testing.assert_array_equal(conv_mem, conv_dmem)
    np.testing.assert_array_equal(iters_mem, iters_dmem)
...
def test_pickle_roundtrip(rsc_d5_data):
    """MemBPDecoder must survive a pickle roundtrip (its _decoder is excluded
    from __getstate__ and rebuilt in __setstate__)."""
    import pickle
    ...
    syndromes = rsc_d5_data["syndromes"]
    ehat_before, conv_before, iters_before = dec.decode_batch_detailed(syndromes)

    dec_restored = pickle.loads(pickle.dumps(dec))
    assert isinstance(dec_restored, MemBPDecoder)
    ehat_after, conv_after, iters_after = dec_restored.decode_batch_detailed(syndromes)

    np.testing.assert_array_equal(ehat_before, ehat_after)
    np.testing.assert_array_equal(conv_before, conv_after)
    np.testing.assert_array_equal(iters_before, iters_after)
