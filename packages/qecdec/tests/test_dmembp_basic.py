"""Basic correctness tests for DMemBPDecoder."""
...
def test_factory_registration(rsc_d5_data, gamma_array):
    assert "DMemBP" in DECODERS_REGISTRY
    assert "DMemBP" in ITERATIVE_DECODERS_REGISTRY
    dec = create_decoder(
        "DMemBP",
        pcm=rsc_d5_data["pcm"],
    ...
    assert isinstance(dec, DMemBPDecoder)
...
def test_gamma_zero_matches_standard_bp(rsc_d5_data):
    """DMemBP with gamma = 0 reduces to standard BP. With norm=1.0 the two
    decoders should produce bit-identical outputs."""
    zero_gamma = np.zeros(rsc_d5_data["num_vars"], dtype=np.float64)
    dmem = DMemBPDecoder(
        rsc_d5_data["pcm"],
        rsc_d5_data["prior"],
        gamma=zero_gamma,
    ...
    bp = BPDecoder(rsc_d5_data["pcm"], rsc_d5_data["prior"], max_iter=30, norm=1.0)
    syndromes = rsc_d5_data["syndromes"]
    ...
    np.testing.assert_array_equal(ehat_dmem, ehat_bp)
    np.testing.assert_array_equal(conv_dmem, conv_bp)
    np.testing.assert_array_equal(iters_dmem, iters_bp)
